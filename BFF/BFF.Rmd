...
BFF_duplicates <- ortho_match_BFF %>% group_by(Status) %>% filter(Status=="Duplicated")
DF_duplicates <- ortho_match_DF %>% group_by(Status) %>% filter(Status=="Duplicated")


# subset buscos with more than 2 gene copies
BFF_dup_more_than2_copies <- ortho_match_BFF %>% group_by(Busco_id) %>% filter(n() > 2)
DF_dup_more_than2_copies <- ortho_match_DF %>% group_by(Busco_id) %>% filter(n() > 2)
...
## check shared/unique genes - GO
## analyze duplicates
## run busco on cat,dog
## get orthologs for cat, dog
