...
buscos_dups <- bind_rows(
                 count_dups(shared_dups, "Shared_dups"),
                 count_dups(uniq_dups_BFF, "Unique_BFF_dups"),
                 count_dups(uniq_dups_DF, "Unique_D_dups")
)

# plot shared/uniq duplicates for both species




# subset buscos with more than 2 gene copies
BFF_dup_more_than2_copies <- BFF_duplicates %>% group_by(Busco_id) %>% filter(n() > 2)
DF_dup_more_than2_copies <- DF_duplicates %>% group_by(Busco_id) %>% filter(n() > 2)
