...
unnested <- split_processes %>% 
            unnest() %>% 
            dplyr::rename(GOID = "BiologicalProcesses")

goterm_definitions <- select(GO.db, keys=unnested$GOID, columns = c("TERM", "ONTOLOGY", "DEFINITION"))

final_go_table <- left_join(unnested, goterm_definitions, by = "GOID") %>% distinct()
