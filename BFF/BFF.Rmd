...
buscos_dups <- bind_rows(
                 ...
                 count_dups(uniq_dups_DF, "Unique_DF_dups")
)
...
# plot all duplicates counts
buscos_dups %>%
  summarize_at("Busco_id", count) #%>%
  
  ggplot(aes(x = Type, y = busco_num, fill = factor(Type))) +
  geom_bar(stat = "Identity", position = "dodge") +
  scale_fill_grey(name = "Species",
                  labels = c("Shared duplicates","Unique - M.nigripes", "Unique - M.putorius furo")) +
  labs(title = "Duplicated BUSCOs shared and unique for each ferret genome assembly",
...
#### Analysis of the multiple duplicates
